%SIGNALCYCLES approximates the second derivative of a signal using
%differences
%   CYCLES = signalCycles(SIGNAL) returns the approximate second derivative
%   of SIGNAL
...
function cycles = signalCycles(signal)
    cycles = diff(signal);
    cycles = abs(cycles) ./ cycles;
    for i = length(cycles) - 1:-1:1
        if cycles(i) == 0
            cycles(i) = cycles(i + 1);
        end
    end
