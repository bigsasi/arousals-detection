%UPDATEWITHPOWERBETA Update the event using beta power values
%   EVENT = updateWithPowerBeta(EVENT, EEG, POWER) returns the  event with 
...
function event = updateWithPowerBeta(event, eeg, power)
    logger = logging.Logger.getLogger('UpdateWithPowerBeta');
    
    powerRate = eeg.eventsRate;
    threshold = Configuration.instance().updateWithPowerBetaThreshold;
    
    limitTime = Configuration.instance().updateWithPowerBetaLimitTime;
    increaseTime = Configuration.instance().updateWithPowerBetaIncreaseTime;
    
    previousSeconds = Configuration.instance().updateWithPowerBetaPreviousSeconds;
