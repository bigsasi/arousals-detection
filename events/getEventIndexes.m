...
%   [INIT, ENDS] = GETEVENTINDEXES(EVENTS) returns both the initial point
%   and the ending point for the events marked in events array. 
...
function [init, ends] = getEventIndexes(events)
    ...
    if not(isempty(init) || isempty(ends))
        % Fix signal starting or ending with an event
        if ends(1) < init(1)
            ends = ends(2:end);
        end
